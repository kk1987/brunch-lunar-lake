...
if [ ! -d "./kernels/$default_kernel" ]; then
	default_kernel="$(echo "$kernels" | tr ' ' '\n' | grep -v '^chromebook-\|^experimental-' | sort -V | tail -1)"
...
if [ "$kernel" == "6.6" ] || [ "$kernel" == "6.12" ] || [ "$kernel" == "7.1" ]; then
