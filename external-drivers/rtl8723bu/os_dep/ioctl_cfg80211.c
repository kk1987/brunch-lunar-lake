...
/* Linux 7.1 changed several cfg80211_ops (key/station handlers) to take a
 * struct wireless_dev * instead of a struct net_device *. */
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(7, 1, 0))
		cfg80211_new_sta(padapter->rtw_wdev, GetAddr2Ptr(pmgmt_frame), &sinfo, GFP_ATOMIC);
#else
